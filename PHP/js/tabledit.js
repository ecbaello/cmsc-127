...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][3] !== 'undefined') {
                                ...
                                if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                    var data = settings.columns.editable[i][2];
                                    $.each(jQuery.parseJSON(settings.columns.editable[i][2]), function(index, value) {
                                        input += index+'="'+value+'" ';
                                    });
                                }
                                input += ' name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;"';
                                if (input.includes('type=') === false) {
                                    input +=  ' type="text"';
                                }
                                input +=  ' disabled>';
                            }
        ...
        function ajax(action)
        {
            ...
            var data = settings.action+'=' + action;

            var willAppend = settings.url.indexOf('?') > -1;

            $.ajax({
               url: settings.url+(willAppend?'&':'?')+data,
               data: $('form.tabledit-form').serializeArray(),
